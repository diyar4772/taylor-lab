...
% Derleme:  pdflatex -interaction=nonstopmode taylor-teori.tex   (üç kez)
...
% =====================================================================
\section{Katsayılar: \texorpdfstring{$n!$}{n!} bir süsleme değildir}
% =====================================================================
...
yaklaşık $2200$ nokta (platforma göre $2224$--$2232$) sınırı \emph{aşıyor görünür}. Örneğin $f=\exp$, $a=1$, $N=12$,
$x=0{,}996$ için
\[
...
\item \textbf{Oran testi her zaman çalışmaz.} $f = 1/(1+x^2)$, $a=1$ için
      $i$ ve $-i$, $a=1$'e \emph{eşit uzaklıkta ama farklı yönlerdedir};
      katsayılar $c_n=(-1)^n 2^{-(n+1)/2}\sin\frac{(n+1)\pi}{4}$ biçiminde
      salınır ve $|a_n/a_{n+d}|$ oranının limiti yoktur: $N=52,\dots,60$ için
      oran testi sırasıyla $\sqrt2,1,2,2,\sqrt2,1,2,2,\sqrt2$ verir. Aynı
      aralıkta C--H $1{,}423$--$1{,}424$'te (\%0{,}6--0{,}7 sapma) sabit
      kalır. Teoremin neden limit değil $\limsup$ ile kurulduğu tam olarak
      budur. (Katsayılar tam aritmetikle üretilmelidir: merkez kayan noktalı
      verildiğinde yüksek mertebeli katsayılar bozulur ve sahte bir sapma
      görülür.)
\end{enumerate}

% =====================================================================
\section{Pürüzsüz ama analitik değil: \texorpdfstring{$C^\omega \subsetneq C^\infty$}{Cω ⊊ C∞}}
